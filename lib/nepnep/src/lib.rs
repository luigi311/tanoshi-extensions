...
macro_rules! generate {
    ($struct_name:ident) => {
        register_extension!($struct_name);

        impl Extension for $struct_name {
            fn detail(&self) -> Source {
                let default = nepnep::detail();
                Source {
                    id: ID,
                    name: NAME.to_string(),
                    url: URL.to_string(),
                    version: Version::from_str(std::env!("CARGO_PKG_VERSION")).unwrap_or_default(),
...
pub fn detail() -> Source {
    Source {
        lib_version: VERSION.to_owned(),
        need_login: false,
...
pub fn filters() -> ExtensionResult<Option<Filters>> {
    ExtensionResult::ok(None)
}

pub fn get_manga_list(source_id: i64, url: &str, param: Param) -> ExtensionResult<Vec<Manga>> {
    let vm_dir = {
        let resp = Request::get(format!("{}/search", url).as_str()).call();
        if resp.status > 299 {
...
/// Get the rest of details unreachable from `get_mangas`
pub fn get_manga_info(source_id: i64, url: &str, path: String) -> ExtensionResult<Manga> {
...
pub fn get_chapters(source_id: i64, url: &str, path: String) -> ExtensionResult<Vec<Chapter>> {
    let resp = Request::get(format!("{}{}", url, &path).as_str()).call();
    if resp.status > 299 {
        return ExtensionResult::err("http request error");
    }
...
pub fn get_pages(url: &str, path: String) -> ExtensionResult<Vec<String>> {
    let resp = Request::get(format!("{}{}", url, &path).as_str()).call();
    if resp.status > 299 {
        return ExtensionResult::err("http request error");
    }
...
mod test {
    ...
    fn test_get_manga_list() {
        let res = get_manga_list(3, "https://mangasee123.com", Param::default());
    ...
    #[test]
    fn test_get_manga() {
        let res = get_manga_info(
            3,
            "https://mangasee123.com",
            "/manga/Kanojo-mo-Kanojo".to_string(),
        );
