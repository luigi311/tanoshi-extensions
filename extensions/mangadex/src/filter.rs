...
// Ordered (label, MangaDex ID) pairs drive both the UI and query conversion.
// Labels are persisted by the host; preserve their spelling and order.
const TAGS: &[(&str, &str)] = &[
...
pub fn tag_id(label: &str) -> Option<&'static str> {
    TAGS.iter()
        .find(|(name, _)| *name == label)
        .map(|(_, id)| *id)
}

fn text_filter(name: &str) -> Input {
    Input::Text {
        name: name.to_string(),
        state: None,
    }
...
fn tag_mode(name: &str, selected: i64) -> Input {
    Input::Select {
        name: name.to_string(),
        values: vec![
...
fn checkbox_group(name: &str, options: &[(&str, bool)]) -> Input {
    Input::Group {
        name: name.to_string(),
        state: options
            .iter()
            .map(|(label, selected)| Input::Checkbox {
...
pub static TITLE_FILTER: LazyLock<Input> = LazyLock::new(|| text_filter("Title"));
// Author and Artist accept comma-separated MangaDex IDs, not display names.
pub static AUTHOR_FILTER: LazyLock<Input> = LazyLock::new(|| text_filter("Author"));
pub static ARTIST_FILTER: LazyLock<Input> = LazyLock::new(|| text_filter("Artist"));
pub static YEAR_FILTER: LazyLock<Input> = LazyLock::new(|| text_filter("Year"));
pub static INCLUDED_TAGS_MODE: LazyLock<Input> =
    LazyLock::new(|| tag_mode("Included Tags Mode", 0));
pub static EXCLUDED_TAGS_MODE: LazyLock<Input> =
    LazyLock::new(|| tag_mode("Excluded Tags Mode", 1));
pub static STATUS_FILTER: LazyLock<Input> = LazyLock::new(|| {
    checkbox_group(
        "Status",
        &[
            ("ongoing", true),
...
pub static CONTENT_RATING_FILTER: LazyLock<Input> = LazyLock::new(|| {
    checkbox_group(
        "Content Rating",
        &[
            ("safe", true),
...
pub static TAG_FILTERS: LazyLock<Input> = LazyLock::new(|| Input::Group {
    name: "Tags".to_string(),
    state: TAGS
        .iter()
        .map(|(label, _)| Input::State {
            name: (*label).to_string(),
            selected: None,
        })
        .collect(),
});
pub static FILTER_LIST: LazyLock<Vec<Input>> = LazyLock::new(|| {
    vec![
        TITLE_FILTER.clone(),
        AUTHOR_FILTER.clone(),
        ARTIST_FILTER.clone(),
